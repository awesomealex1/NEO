...
class LAME(nn.Module):
    """
    LAME: Laplacian Regularization Adaptation Method
    """  
    def __init__(self, model: VisionTransformer, knn=5, sigma=1.0, affinity='kNN', force_symmetry=True):
        super().__init__()
        self.featurer = get_vit_featurer(copy.deepcopy(model))
        ...
        self.imagenet_mask = None  # keeping consistency with Tent/CoTTA

    def forward(self, x, adapt: bool = True):
        # --- always get base model outputs ---
        with torch.no_grad():
            feats = self.featurer(x)
        ...
        if not adapt:
            # Just return plain softmax prediction
            return out.softmax(dim=1)
        # --- adaptation path (Laplacian optimization) ---
