...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema, imagenet_mask):
    ...
    if hasattr(second_output, "logits"):
        second_output = second_output.logits
    if imagenet_mask is not None:
        second_output = second_output[:, imagenet_mask]
    entropys2 = softmax_entropy(second_output)
