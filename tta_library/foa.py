"""
Adapted from FOA Authors ICML 2024
"""
...
class FOA(nn.Module):
    ...
    """
    def __init__(self, model:PromptViT, fitness_lambda=0.4, population_size=27):
        super().__init__()
        self.fitness_lambda = fitness_lambda

        self.model = model
        self.feature_dim = model.prompt_dim
    ...
    def _get_shift_vector(self):
        ...
        if self.hist_stat is None:
            ...
            return self.train_info[1][-self.feature_dim:] - self.hist_stat
    ...
    def forward(self, x, adapt=True):
        ...
        for j, prompt in enumerate(prompts):
            self.model.prompts = torch.nn.Parameter(torch.tensor(prompt, dtype=torch.float).
                                                        reshape_as(self.model.prompts).to(get_device()))
            ...
            batch_means.append(batch_mean[-self.feature_dim:].unsqueeze(0))
            del batch_mean
...
criterion_mse = nn.MSELoss(reduction='none').to(get_device())
...
def forward_and_get_loss(images, model:PromptViT, fitness_lambda, train_info, shift_vector, imagenet_mask):
    ...
    cls_features = features[:, -model.prompt_dim:] # the feature of classification token
