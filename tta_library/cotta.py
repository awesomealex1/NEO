...
class CoTTA(nn.Module):
    ...
    - forward(x, adapt=True): perform adaptation + prediction
    - forward(x, adapt=False): pure inference, no updates
    """
    ...
    def forward(self, x, adapt: bool = True):
        if self.episodic:
            self.reset()

        if not adapt:
            # Pure inference, no adaptation
            self.model.eval()
            with torch.no_grad():
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        if hasattr(anchor_prob, "logits"):
            anchor_prob = anchor_prob.logits
        if self.imagenet_mask is not None:
            anchor_prob = anchor_prob[:, self.imagenet_mask]
        anchor_prob = torch.nn.functional.softmax(anchor_prob, dim=1).max(1)[0]

        standard_ema = self.model_ema(x)
        if hasattr(standard_ema, "logits"):
            standard_ema = standard_ema.logits
        if self.imagenet_mask is not None:
        ...
        if to_aug: 
            for _ in range(N):
                ...
                if hasattr(outputs_, "logits"):
                    outputs_ = outputs_.logits
                if self.imagenet_mask is not None:
        ...
        for nm, m  in self.model.named_modules():
            for npp, p in m.named_parameters():
                if npp in ['weight', 'bias'] and p.requires_grad:
                    mask = (torch.rand(p.shape) < 0.001).float().to(get_device())
                    with torch.no_grad():
